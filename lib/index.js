...
async function writeMultiAuthRecord(path, record3) {
  const parent = dirname(path);
  try {
    await prepareParent(parent);
...
function createCredentialCoordinator(options) {
  ...
  const coordinator = {
    credential: async (signal, credentialOptions) => {
      ...
      const record3 = await options.store.read();
      if (record3 === void 0) {
      ...
      if (credentialOptions?.forceRefresh !== true && cached !== void 0 && sameRecord(cached, record3) && isFresh(cached.value, now(), refreshLeadMs)) {
        state = "logged-in";
        errorCode2 = void 0;
    ...
    replaceFromLogin: (credential, record3) => {
      if (disposed) return;
      generation += 1;
      abortOperations();
      cached = { value: { ...credential }, revision: record3.revision, ...record3.lineage === void 0 ? {} : { lineage: record3.lineage } };
      observedRevision = record3.revision;
      observedLineage = record3.lineage;
      state = "logged-in";
      errorCode2 = void 0;
    ...
    status: async () => {
      ...
      return makeCredentialStatus(record3);
    },
    ...
    logout: async () => {
      ...
      let cleared = true;
      if (record3 !== void 0) {
        cleared = await options.store.clearIfCurrent(record3.revision, record3.lineage);
      }
      if (!isActive(logoutGeneration)) return { state: "logged-out" };
      if (!cleared) {
    ...
    revoke: async (confirmed, signal) => {
      ...
      const flight = revokeCredential(record3, revokeGeneration);
      revokeFlight = flight;
      void flight.then(
    ...
    selectAccount: async (id) => {
      ...
      refreshFlight = void 0;
      cached = void 0;
      const record3 = await options.store.selectAccount(id);
      if (record3) {
        observe(record3);
      } else {
        clearObservedCredential();
  ...
  function observe(record3) {
    const lineageChanged = observedRevision !== 0 && observedLineage !== record3.lineage;
    const recordChanged = observedRevision !== 0 && (observedRevision !== record3.revision || lineageChanged);
    if (recordChanged) {
      cached = void 0;
      state = "logged-in";
      errorCode2 = void 0;
  ...
  function sameRecord(value, record3) {
    return value.revision === record3.revision && value.lineage === record3.lineage;
  }
  function makeCredentialStatus(record3) {
    const configured = record3 !== void 0;
  ...
  async function refreshCredential(startGeneration) {
    ...
    let record3 = await options.store.read();
    ...
    for (let attempt = 0; attempt < MAX_REFRESH_ATTEMPTS; attempt += 1) {
      ...
      try {
        result2 = await runBounded(
          (signal) => refreshAccessToken({ refreshToken: record3.refreshToken, signal }),
          operation,
          operationTimeoutMs
        );
      ...
      if (!isActive(startGeneration)) return void 0;
      const responseRefreshToken = result2.refreshToken ?? record3.refreshToken;
      if (current === void 0) {
        clearObservedCredential();
        return void 0;
      }
      if (sameLineage(current, record3) && current.revision !== record3.revision) {
      ...
      if (!sameLineage(current, record3)) {
        record3 = current;
        observe(record3);
        continue;
      }
      const committed = await options.store.compareAndCommit(
        record3.revision,
        {
          ...
          ...record3.email === void 0 ? {} : { email: record3.email },
          ...record3.lineage === void 0 ? {} : { lineage: record3.lineage }
        },
        record3.lineage
      );
      ...
      if (latest.refreshToken === responseRefreshToken && sameLineage(latest, record3)) {
  ...
  async function adoptRefreshedCredential(result2, record3, startGeneration) {
    if (!isActive(startGeneration)) return void 0;
    const credential = {
      accessToken: result2.accessToken,
      refreshToken: result2.refreshToken ?? record3.refreshToken,
  ...
  async function revokeCredential(record3, revokeGeneration) {
    const operation = beginOperation();
    try {
      await runBounded(
        (signal) => revokeGrant({ token: record3.refreshToken, signal }),
        operation,
        operationTimeoutMs
      );
    } catch (error) {
...
function createWireIdentity() {
  ...
  const serialize = (url, request2) => {
    ...
    const lines = pairs.map(([name3, value]) => `${name3}: ${value}`).join("\r\n");
    const head = Buffer2.from(`POST ${parsed.pathname}${parsed.search} HTTP/1.1\r
${lines}\r
\r
`);
    if (!pairs.some(([name3]) => name3 === "Transfer-Encoding")) return body.byteLength === 0 ? head : Buffer2.concat([head, body]);
...
function assertSafeHeaderValue(name3, value, code) {
  if (typeof value !== "string" || value.length === 0 || value.length > MAX_HEADER_VALUE_LENGTH) {
    throw new WireIdentityError(code, `${name3} is outside the safe header-value bounds`);
  }
  for (let index = 0; index < value.length; index += 1) {
    const codePoint = value.charCodeAt(index);
    if (codePoint < 32 || codePoint === 127) {
      throw new WireIdentityError(code, `${name3} contains an unsafe header character`);
    }
  }
}
...
function parseResponseHead(value) {
  ...
  for (const line of lines) {
    ...
    const name3 = line.slice(0, separator);
    const raw = line.slice(separator + 1).trim();
    if (!/^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/u.test(name3) || hasInvalidHeaderValue(raw)) {
...
function gateSubject(record3) {
  return record3.lineage ?? "legacy-account";
}
  ...
  "src/antigravity/auth-service.ts"() {
    ...
    AntigravityAuthService = class {
      ...
      async status() {
        ...
        const phase = flowStatus.phase === "idle" && record3 !== void 0 ? "success" : flowStatus.phase;
        const maskedEmail = maskEmail(record3?.email);
        const credentialStatus = await this.credentials.status();
        const gateEvidence = await this.gateEvidenceFor(record3);
        const login = {
          phase,
          configured: record3 !== void 0,
          projectAvailable: record3?.projectId !== void 0,
      ...
      async recordLlmFamilyGate(family, outcome) {
        const record3 = await this.requireRecord();
        const subject = gateSubject(record3);
      ...
      async requireRecord() {
        const record3 = await this.readRecord();
        if (record3 === void 0) throw new OAuthFlowError("internal", "Antigravity login is required");
        return record3;
      }
      async readRecord() {
        try {
...
function cleanModelDisplayName(name3) {
  return name3.replace(/\s*\([^)]*\)\s*$/u, "").trim();
}
...
function hardenClaudeToolDeclaration(value) {
  ...
  const parameters = Object.entries(properties).map(([name3, property]) => {
    const requiredHint = required.has(name3) ? ", REQUIRED" : "";
    return `${name3} (${claudeToolTypeHint(property)}${requiredHint})`;
  });
  return {
    ...value,
...
function claudeToolTypeHint(value) {
  ...
  if (type === "array") {
    ...
    const nested = Object.entries(value.items.properties).map(([name3, property]) => {
      const nestedType = isRecord8(property) && typeof property.type === "string" ? property.type : "unknown";
      return `${name3}: ${nestedType}${nestedRequired.has(name3) ? " REQUIRED" : ""}`;
    });
  ...
  if (type === "object" && isRecord8(value.properties)) {
    ...
    const nested = Object.entries(value.properties).map(([name3, property]) => {
      const nestedType = isRecord8(property) && typeof property.type === "string" ? property.type : "unknown";
      return `${name3}: ${nestedType}${nestedRequired.has(name3) ? " REQUIRED" : ""}`;
    });
...
function rememberToolName(toolNames, callId, name3) {
  if (name3.length === 0 || name3.length > 256 || containsControl(name3)) throw new LlmError("The tool call name is invalid", "INVALID_ARGS");
  const id = requireToolCallId(callId);
  const existing = toolNames.get(id);
  if (existing !== void 0 && existing !== name3) throw new LlmError("A tool call id was reused with a different name", "INVALID_ARGS");
  toolNames.set(id, name3);
  return id;
}
function requireToolName(toolNames, callId) {
  const name3 = toolNames.get(requireToolCallId(callId));
  if (name3 === void 0) throw new LlmError("A tool result did not match a prior tool call", "INVALID_ARGS");
  return name3;
}
...
function parsePart(value) {
  ...
  if (functionCall !== void 0) {
    const name3 = stringValue(functionCall.name);
...
async function readProviderFailureDetails(response, options) {
  let message;
  ...
  try {
    ...
    return { contextWindowExceeded: false, capacityUnavailable, ...message === void 0 ? {} : { message } };
  } catch (error) {
...
function providerReportsCapacityUnavailable(payload, wireModel) {
  if (payload.length > MAX_PROVIDER_ERROR_BYTES || !jsonDepthIsBounded(payload, MAX_PROVIDER_ERROR_JSON_DEPTH)) return false;
  try {
    const value = JSON.parse(payload);
    if (!isRecord8(value) || !isRecord8(value.error)) return false;
    return value.error.code === 503 && value.error.status === "UNAVAILABLE" && value.error.message === `No capacity available for model ${wireModel} on the server`;
  } catch {
    return false;
  }
}
function providerErrorEnvelopeMessage(value, depth) {
  if (depth > MAX_PROVIDER_ERROR_JSON_DEPTH || value.length === 0 || value.length > MAX_PROVIDER_ERROR_BYTES) return void 0;
  ...
  try {
    ...
    return isRecord8(parsed) ? providerErrorMessage(parsed, depth + 1) : void 0;
  } catch {
    return safeProviderErrorMessage(json);
  }
}
function providerErrorMessage(value, depth) {
  if (depth > MAX_PROVIDER_ERROR_JSON_DEPTH) return void 0;
...
function safeProviderErrorMessage(value) {
  const sanitized = value.replace(/Bearer\s+\S+/giu, "Bearer [redacted]").replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/giu, "[redacted-email]").replace(/[A-Za-z0-9+/_=-]{48,}/gu, "[redacted]").replace(/[\u0000-\u001f\u007f]+/gu, " ").replace(/\s+/gu, " ").trim().slice(0, 1024);
...
function toLlmError(error) {
  ...
  if (error instanceof PrivateTransportError) {
    ...
    const messages = {
      ...
      upstream: `Antigravity upstream service is temporarily unavailable${error.status === void 0 ? "" : ` (HTTP ${error.status})`}; please retry later.`,
...
async function admitImageBytes(options, data, source, name3 = "antigravity-image", signal) {
...
async function admitBase64Image(options, encoded, source = "inline", name3, signal) {
  ...
  return admitImageBytes(options, data, source, name3, signal);
}
...
function parseImageRef(value) {
  ...
  const name3 = typeof value.name === "string" && isBoundedSafeText(value.name, 256) ? safeName(value.name) : void 0;
  return {
...
function safeName2(value) {
  const name3 = value.replaceAll("\\", "/").split("/").at(-1)?.slice(0, 128);
  return name3 !== void 0 && isBoundedSafeText(name3, 128) ? name3 : "live-gate-output.png";
}
...
function assertVaultRecord(record3) {
  if (record3?.kind !== "grant" || record3.payload?.version !== VERSION || typeof record3.payload.activeId !== "string" || !Array.isArray(record3.payload.accounts) || record3.payload.accounts.length === 0) throw new Error("Codex account vault contains a malformed grant record");
  const ids = /* @__PURE__ */ new Set();
  const accounts = record3.payload.accounts.map((account) => {
  ...
  if (!ids.has(record3.payload.activeId)) throw new Error("Codex account vault active account is missing");
  const legacyAccountId = record3.payload.legacyAccountId;
  if (legacyAccountId !== void 0 && !ids.has(legacyAccountId)) throw new Error("Codex account vault legacy account is missing");
  return {
    version: VERSION,
    activeId: record3.payload.activeId,
    legacyAccountId,
    accounts
  };
}
  ...
  list() {
    return this.#enqueue(async () => {
      ...
      return payload.accounts.map((account) => {
        ...
        const subscriptionUntil = typeof authObj?.chatgpt_subscription_active_until === "string" ? authObj.chatgpt_subscription_active_until : cred.subscriptionUntil;
        return {
          id: account.id,
          label: account.label,
  ...
  return async (endpoint, payload, signal) => {
    try {
      ...
      if (endpoint === "login/start") {
        const method = input.method ?? "browser";
...
function fetchThroughProxy(input, init, proxyUrl) {
  ...
  return new Promise((resolve3, reject) => {
    const request$1 = request(target, {
      ...
      for (const [name3, value] of Object.entries(response.headers)) if (Array.isArray(value)) value.forEach((item) => responseHeaders.append(name3, item));
      else if (value !== void 0) responseHeaders.set(name3, value);
...
function classifyTransportError(error) {
  const name3 = error?.name;
  const code = String(error?.code ?? error?.cause?.code ?? "");
  if (name3 === "AbortError" || name3 === "TimeoutError" || /ETIMEDOUT|UND_ERR_CONNECT_TIMEOUT/u.test(code)) return "timeout";
...
var OriginalImageStore = class {
  ...
  async save(sessionId, data, name3 = "codex-generated-original.png") {
...
function limitOf(id, name3, value) {
  if (value === void 0 || value === null) return void 0;
...
function resetCreditsOf(value) {
  ...
  const available = (value.credits ?? []).filter((credit) => record$1(credit) && credit.status?.toLowerCase?.() === "available").map((credit) => {
    const name3 = typeof credit.title === "string" && credit.title.trim().length > 0 ? credit.title.trim().slice(0, 120) : void 0;
    let expiresAt;
...
function retryAfterMs(response, now, maximum) {
  ...
  const delay2 = Number.isFinite(seconds) && seconds >= 0 ? seconds * 1e3 : Date.parse(raw) - now();
  if (!Number.isFinite(delay2) || delay2 < 0) return void 0;
  return Math.min(delay2, maximum);
}
    ...
    read({ force = false, signal } = {}) {
      ...
      const current = load(signal).then((value) => {
        ...
        if (generation === currentGeneration && error?.name !== "AbortError") {
          const delay2 = Number.isFinite(error?.retryAfterMs) ? error.retryAfterMs : failureTtlMs;
          failed = {
...
function observeQuotaForecast(state, windows, now = Date.now(), context = {}) {
  ...
  for (const window of windows ?? []) {
    ...
    const record3 = resetChanged || quotaIncreased || observationGap ? {
      resetsAt,
      samples: []
    } : {
      resetsAt,
    ...
    const latest = record3.samples.at(-1);
    if (latest === void 0 || now > latest.at) {
      record3.samples.push({
        at: now,
        remainingPercent
      });
      record3.samples = record3.samples.filter((sample) => sample.at >= now - HISTORY_MS).slice(-192);
      changed = true;
    }
    next.windows[key] = record3;
  }
  return {
    state: next,
    changed
...
function estimateQuotaForecast(state, window, now = Date.now(), context = {}) {
  ...
  const record3 = state?.windows?.[keyFor(window, context)];
  if (record3 === void 0) return { status: "calibrating" };
  const resetsAt = finite(window.resetsAt) ? Number(window.resetsAt) : null;
  if (record3.resetsAt === null !== (resetsAt === null) || resetsAt !== null && Math.abs(record3.resetsAt - resetsAt) > 300) return { status: "calibrating" };
  let samples = record3.samples.filter((sample) => sample.at >= now - 2 * HOUR_MS && sample.at <= now);
...
function sanitize(value) {
  ...
  const windows = {};
  for (const [key, record3] of entries) {
    if (key.length === 0 || key.length > 320 || record3 === null || typeof record3 !== "object" || !Array.isArray(record3.samples) || record3.samples.length > MAX_SAMPLES) return void 0;
    const resetsAt = record3.resetsAt === null ? null : Number(record3.resetsAt);
    if (resetsAt !== null && !Number.isFinite(resetsAt)) return void 0;
    const samples = [];
    for (const sample of record3.samples) {
...
import {
  LlmAdapter as LlmAdapter2,
  LlmError as LlmError6,
  ReasoningEffortId as ReasoningEffortId2,
...
import { CONTEXT_WINDOW_EXCEEDED_CODE as CONTEXT_WINDOW_EXCEEDED_CODE2, EMPTY_RESPONSE_CODE, isContextWindowExceededError, isQuotaExceededError, LlmError as LlmError42, QUOTA_EXCEEDED_CODE } from "@deepseek-ai/dsh-llm";
...
import { attributionHeaders, LlmError as LlmError52 } from "@deepseek-ai/dsh-llm";
...
var __knownSymbol = (name22, symbol) => (symbol = Symbol[name22]) ? symbol : /* @__PURE__ */ Symbol.for("Symbol." + name22);
var __typeError = (msg) => {
  throw TypeError(msg);
};
    ...
    switch (block.type) {
      ...
      case "tool-call":
        content.push({
          ...
          id: block.id,
          name: block.name,
          arguments: parseArguments2(block.arguments)
        });
        break;
      case "image":
  ...
  for (const message of split.messages) {
    ...
    for (const result2 of results) {
      messages.push({
        ...
        toolName: toolNames.get(result2.toolCallId) ?? "unknown",
        content: [{
          type: "text",
          text: toolResultText(result2.content) || "(no output)"
        }],
...
async function toPiContextWithImages(options, images, onReplayDegrade) {
  ...
  for (const message of exactMessages) {
    ...
    for (const result2 of results) {
      const resultContent = await userContent(result2.content, requestImages, resolveImageAccess);
      messages.push({
        role: "toolResult",
        toolCallId: result2.toolCallId,
        toolName: toolNames.get(result2.toolCallId) ?? "unknown",
  ...
  if (piAiOverflow || harnessOverflow) {
    return {
      ...
      failure: {
        ...
        code: CONTEXT_WINDOW_EXCEEDED_CODE2
      }
    };
  }
...
async function* toStreamChunks(events, contextWindow, callerSignal, requestedModel) {
  ...
  for await (const event of events) {
    switch (event.type) {
      ...
      case "toolcall_start": {
        ...
        const name22 = partial?.type === "toolCall" ? partial.name : "";
        toolIds.set(event.contentIndex, { id, name: name22 });
  ...
  throw new LlmError42("pi-ai event stream ended without done/error", "STREAM_CLOSED");
}
...
var LEVELS2 = ["off", "minimal", "low", "medium", "high", "xhigh", "max"];
function thinkingLevels(metadata, known) {
  ...
  for (const item of Array.isArray(metadata.reasoning_options) ? metadata.reasoning_options : []) {
    ...
    for (const value of option.values) {
      ...
      if (LEVELS2.includes(level)) map[level] = String(value);
    }
  }
...
function readModelMetadata(body, baseURL, builtin) {
  const provider = record2(record2(body)["opencode-go"]);
  ...
  const entries = record2(provider.models);
  const models = /* @__PURE__ */ new Map();
  const errors = /* @__PURE__ */ new Map();
  const details = /* @__PURE__ */ new Map();
  for (const [id, value] of Object.entries(entries)) {
    const data = record2(value);
    details.set(id, {
    ...
    try {
      ...
      const npm = record2(metadata.provider).npm ?? provider.npm;
      ...
      const limit = record2(metadata.limit);
      const input = record2(metadata.modalities).input;
      ...
      const family = typeof metadata.family === "string" ? Object.keys(entries).find((key) => record2(entries[key]).family === metadata.family && builtin.get(key)?.api === api2) : void 0;
      const known = exact?.api === api2 ? exact : family === void 0 ? void 0 : builtin.get(family);
      const compat = api2 === "openai-completions" ? {
        supportsStore: false,
        supportsDeveloperRole: false,
        maxTokensField: "max_tokens",
        ...record2(metadata.interleaved).field === "reasoning_content" ? { requiresReasoningContentOnAssistantMessages: true } : {},
        ...known?.compat
      } : api2 === "openai-responses" ? { sessionAffinityFormat: "openai-nosession", ...known?.compat } : { ...known?.compat };
      const cost = rates(metadata.cost);
      const tiers = record2(metadata.cost).tiers;
      if (Array.isArray(tiers)) {
        cost.tiers = tiers.flatMap((item) => {
          const tier = record2(record2(item).tier);
      ...
      models.set(id, {
        ...
        contextWindow: positiveInteger2(limit.context, "context limit"),
        maxTokens: positiveInteger2(limit.output, "output limit"),
        cost,
        compat
      });
    } catch (error) {
...
async function fetchLiveModelIds(baseURL) {
  ...
  let result2;
  try {
    result2 = await fetchJsonResponse(url, {
      method: "GET",
      cache: "no-cache",
    ...
    throw new LlmError52(`could not ${action} ${endpoint}: ${detail}`, "DISCOVERY_FAILED", { cause: error });
  }
  const { response, body } = result2;
  if (!response.ok) throw new LlmError52(`${endpoint} answered HTTP ${response.status}`, "DISCOVERY_FAILED");
  try {
    return readLiveModelIds(body);
  } catch (error) {
    throw new LlmError52(`${endpoint} returned an invalid model listing: expected a "data" array`, "DISCOVERY_FAILED", { cause: error });
  }
}
...
function listingError(snapshot) {
  const detail = snapshot.listingFailure instanceof LlmError52 ? snapshot.listingFailure.message : "the live model listing is unreachable";
  return new LlmError52(`llm-opencode-go: ${detail}; refresh the model list to retry`, "DISCOVERY_FAILED", { cause: snapshot.listingFailure });
}
...
var fields = {
  ...
  modelVisibility: z5.dict(z5.boolean().required()).default({}),
  apiKeyEnv: z5.string().role("credential-ref").default(DEFAULT_API_KEY_ENV),
  baseURL: z5.string().default(DEFAULT_BASE_URL),
  refreshMinutes: z5.number().step(1).min(1).max(7 * 24 * 60).default(DEFAULT_REFRESH_MINUTES),
  streamIdleTimeoutMs: z5.number().min(Number.MIN_VALUE).max(MAX_TIMER_DELAY_MS).default(DEFAULT_STREAM_IDLE_TIMEOUT_MS),
  ...
  maxRequestImageBytes: z5.number().step(1).min(1).default(DEFAULT_MAX_REQUEST_IMAGE_BYTES),
  requestImagePixelBudget: z5.number().step(1).min(1).default(DEFAULT_REQUEST_IMAGE_PIXEL_BUDGET),
  requestImageMaxBytes: z5.number().step(1).min(1).default(DEFAULT_REQUEST_IMAGE_MAX_BYTES),
  // null explicitly selects catalog values, overriding even inherited profile limits.
  modelLimits: z5.dict(z5.union([z5.const(null), z5.object({
    contextWindow: z5.union([z5.const(null), z5.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER)]),
    maxTokens: z5.union([z5.const(null), z5.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER)])
  })])).default({})
};
var PlainConfig = z5.object(fields);
var Config4 = z5.object(Object.fromEntries(
...
var OpencodeGoAdapter = class extends LlmAdapter2 {
  constructor(options) {
    super();
    this.options = options;
  }
    ...
    if (levels.length > 0) {
      ...
      reasoning.reasoning = {
        efforts: levels.map((level) => ({
          id: ReasoningEffortId2(level),
          name: `${level.charAt(0).toUpperCase()}${level.slice(1)}`
        })),
        ...fallback === void 0 ? {} : { defaultEffort: ReasoningEffortId2(fallback) }
      };
    }
    return {
      provider: PROVIDER_ID,
  ...
  async *stream(options) {
    ...
    try {
      ...
      try {
        ...
        try {
          while (true) {
            const result2 = await watchdog.next(iterator);
...
function parseGoUsage(value) {
  ...
  for (const key of ["rolling", "weekly", "monthly"]) {
    ...
    result2[key] = { status: row.status, percent: row.percent, resetsAt: row.resetsAt };
  }
  return result2;
}
var usageCodec = {
...
var GoUsageService = class extends TypertRemoteService {
  ...
  async read() {
    ...
    if (this.identity?.baseURL !== baseURL || this.identity.key !== key) {
      this.identity = { baseURL, key, source: randomUUID22() };
    }
    const { source } = this.identity;
    const endpoint = diagnosticURL(`${baseURL}/usage`);
    let result2;
    try {
      result2 = await fetchJsonResponse(`${baseURL}/usage`, {
...
function apply6(ctx, raw) {
  const config = raw && typeof raw.enabled === "object" ? raw : Config4(raw);
  ...
  const syncRoute = () => {
    ...
    void credentials.describe(credentialRef2(current().apiKeyEnv)).then((info) => {
  ...
  syncRoute();
  const undiscover = ctx.llm.registerModelDiscovery(name, async (request2) => {
    if (request2.provider !== PROVIDER_ID && !(request2.baseURL ?? "").includes("opencode.ai")) {
      throw new LlmError7(
