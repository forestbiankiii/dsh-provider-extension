...
export declare function effortIndex(model: ModelPanelModel, effortId: string | undefined): number;
/** Only a valid declared default is a known resting effort; never infer Max. */
export declare function restingEffort(model: ModelPanelModel): string | undefined;
/** Match the complete route, not a model id that another provider may also own. */
export declare function isCurrentModel(current: ModelDirectoryState['current'], provider: string, model: ModelPanelModel): boolean;
/** Submit only installed ModelSelection fields. Metadata cannot enable context forwarding. */
export declare function selectionForRow(model: ModelPanelModel, provider: string, effortId: string | undefined): ModelSelection;
/** Render the current provider, or the first loaded group as an explicit fallback. */
export declare function activeGroup(state: ModelDirectoryState): ModelPanelGroup | undefined;
