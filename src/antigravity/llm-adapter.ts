...
/** Provider-neutral replacement for a tool result whose signed call was dropped. */
const UNSIGNED_TOOL_RESULT_NOTE = 'A tool call ran here, but its provider-issued thinking signature is no longer available, so its result cannot be replayed. Use another tool call if you still need this information.'
...
export function buildAntigravityGeneratePayload(options: GenerateOptions, credential: HostCredential): Record<string, unknown> {
  ...
  const contents = options.messages
    ...
    .map(message => mapMessage(message, options.model, toolNames, droppedCallIds))
...
function mapMessage(
  ...
  for (const block of message.content) {
    ...
    if (block.type === 'text') {
      ...
      // A thinking-mode tool call without its provider signature can never be
      // replayed: the wire rejects a bare functionCall with 400 INVALID_ARGUMENT.
      // Drop the call instead of sending a request the provider must refuse.
