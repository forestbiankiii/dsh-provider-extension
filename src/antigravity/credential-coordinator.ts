...
import type { AntigravityAccountRecord, AntigravityAuthRecord, AntigravityAuthStore } from './auth-store.ts'
...
export interface CredentialCoordinator {
  ...
  accounts(): Promise<readonly AntigravityAccountRecord[]>
...
export function createCredentialCoordinator(options: CredentialCoordinatorOptions): CredentialCoordinator {
  ...
  const coordinator: CredentialCoordinator = {
    ...
    selectAccount: async (id: string) => {
      ...
      refreshFlight = undefined
      cached = undefined
      const record = await options.store.selectAccount(id)
      if (record) {
        observe(record)
      } else {
        clearObservedCredential()
    ...
    updateAccount: async (id: string, patch: { label?: string | undefined; tier?: string | undefined }) => {
      ensureNotDisposed()
